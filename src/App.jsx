...
// ---------------- CONFIG ----------------
const MAX = {
  assignment: 10, mst1: 20, mst2: 20, surprise: 12, quiz: 4, attendance: 2,
  endsem_written: 60, case_study: 10, exp1: 30, exp2: 30, exp3: 30, exp4: 30,
  class_performance: 10, course_project: 5, industry_assessment: 10,
  endsem_practical_external: 40, endsem_practical: 40,
};
...
const INPUTS = {
  ...
  hybrid: [
    "assignment","mst1","mst2","surprise","quiz","case_study","attendance",
    "exp1","exp2","exp3","exp4","class_performance","course_project",
...
const LABELS = {
  assignment: "Assignment (out of 10)", mst1: "MST 1 (out of 20)", mst2: "MST 2 (out of 20)",
  surprise: "Surprise Test (out of 12)", quiz: "Quiz (out of 4)", attendance: "Attendance (out of 2)",
  endsem_written: "End Sem Written (out of 60)", case_study: "Case Study (out of 10)",
  exp1: "Experiment 1 (out of 30)", exp2: "Experiment 2 (out of 30)", exp3: "Experiment 3 (out of 30)",
  exp4: "Experiment 4 (out of 30)", class_performance: "Class Performance (out of 10)",
  course_project: "Course Project (out of 5)", industry_assessment: "Industry Assessment (out of 10)",
...
export default function App() {
  ...
  function compute() {
    ...
    if (mode === "theory" || mode === "hybrid") {
      external = (clamp(input.endsem_written, 0, 60) / 60) * EXTERNAL_WEIGHT[mode];
    }
    if (mode === "practical") {
      external = (clamp(input.endsem_practical_external, 0, 40) / 40) * EXTERNAL_WEIGHT.practical;
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-900 to-black p-3 sm:p-6 text-white relative">
      ...
      <style>{`
        ...
        .comet-cursor {
          ...
          background: white; box-shadow: 0 0 12px 4px rgba(255,255,255,0.9);
        ...
        .comet-trail {
          ...
          filter: blur(14px); pointer-events: none; z-index: 9998;
          transform: translate3d(-200px,-200px,0); mix-blend-mode: screen;
        }
      `}</style>

      <div className="max-w-6xl mx-auto backdrop-blur-xl bg-white/5 p-3 sm:p-4 rounded-3xl shadow-2xl border border-white/10 relative">
        ...
        <header className="
          ...
          justify-between
          gap-2 px-3 sm:px-6 py-2 sm:py-4
          bg-gradient-to-r from-indigo-900/30 to-violet-900/10
          border-b border-white/5
        ">
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 via-fuchsia-300 to-emerald-300">
              Internal Marks Calculator
            </h1>
            <div className="text-xs text-white/60 mt-1">By <span className="font-semibold text-indigo-200">MrDnobody</span> · Premium</div>
          </div>
            ...
            {["theory","hybrid","practical"].map((m)=>(
              <button key={m} onClick={()=>{setMode(m); reset();}}
                className={`px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-semibold transition ${
                  mode===m ? "bg-indigo-600 text-white ring-1 ring-indigo-400 scale-105"
        ...
        <main className="p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          {/* INPUT SECTION — FIXED, SCROLLABLE */}
          <section className="
            lg:col-span-2 
          ...
          ">
            {INPUTS[mode].map((k)=>(
              <label key={k}
                className="relative bg-gradient-to-br from-white/6 to-white/3 rounded-xl p-3 sm:p-4 border border-white/6 hover:scale-105 transition group">
                
                <div className="absolute -inset-0.5 rounded-xl opacity-0 group-hover:opacity-100 
                bg-gradient-to-r from-purple-500/8 to-cyan-400/8 blur-xl"></div>
                ...
                <div className="relative z-10">
                  ...
                  <input type="number" min={0} max={MAX[k]} value={input[k]}
                    ...
                    className="mt-1 sm:mt-2 w-full p-2 sm:p-3 rounded-lg bg-black/60 
                    border border-white/8 text-white text-sm sm:text-base"/>
            ...
            {(mode==="theory"||mode==="hybrid") && (
              <div className="bg-gradient-to-br from-yellow-400/6 to-yellow-400/12 rounded-xl 
              p-3 sm:p-4 border border-yellow-200/6">
            ...
            {mode==="practical" && (
              <div className="bg-gradient-to-br from-rose-400/6 to-rose-400/12 rounded-xl 
              p-3 sm:p-4 border border-rose-200/6">
                
                <div className="text-xs sm:text-sm text-white/80">End Sem Practical External (out of 40)</div>
                ...
                <input type="number" min={0} max={40} value={input.endsem_practical_external}
                  ...
                  className="mt-1 sm:mt-2 w-full p-2 sm:p-3 rounded-lg bg-black/60 border border-white/8 
          ...
          {/* RESULT SECTION */}
          <aside className="rounded-2xl p-4 sm:p-6 bg-gradient-to-br from-white/6 to-white/3 
          border border-white/6 shadow-lg flex flex-col justify-between">
              ...
              <div className="mt-4 grid gap-3">
                ...
                <div className="flex items-center justify-between mt-4 pt-3 border-t border-white/5">
                  <div>
                    ...
                    bg-gradient-to-r from-green-300 to-emerald-100">
                      {finalDisplay} / 100
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <button 
                      ...
                      from-indigo-500 to-violet-400 text-white font-semibold shadow hover:scale-105 transition">
                      Compute
                    </button>

                    <button 
                      onClick={reset} 
                      className="px-3 py-2 rounded-full bg-white/6 text-white/80 
            ...
            {result && (
              <div className="mt-4 bg-black/30 p-3 sm:p-4 rounded-lg border border-white/6">
                <div className="text-xs text-white/70 mb-1">Breakdown (internal contributions)</div>

                <div className="text-xs sm:text-sm text-white/90 max-h-40 overflow-auto">
                  {result.rows.map((r)=>(
                    <div key={r.key} className="flex justify-between py-1 
